#' @title Construct a matrix of transcript compatibility counts from alevin 
#' output
#'
#' @description Constructs a UMI count matrix with equivalence class identifiers
#' in the rows and barcode identifiers in thecolumn. The count matrix is 
#' generated from one or multiple `bfh.txt` files that have ben generated by 
#' alevin-fry when run with the --dumpBFH flag. Alevin-fry - 
...
alevinEC <- function(paths, tx2gene, multigene = FALSE, quiet = FALSE){
  ...
  n_tx <- fread(paths[1], nrows=1,
                sep = " ",quote = "", header = FALSE)$V1
  tname <- fread(paths[1], skip = 3, nrows = n_tx, 
                 sep = " ", quote = "", header = FALSE)$V1
...
readBFH <- function(file, tx2gene, multigene){
  ...
  numbers <- fread(file, nrows=3, sep = " ", quote = "", header = FALSE)
  n_tx <- numbers$V1[1]
  n_bc <- numbers$V1[2]
  
  # read barcode names
  bc_id_to_name <- fread(file, skip = 3+n_tx, nrows = n_bc, 
                         sep = " ", quote = "", header = FALSE)$V1
