...
#' Minimal function that load alevin-fry data into pieces for internal use only
#'
#' This was refactored out of the main `loadFry` function to facilitate re-use
#' in unit tests and such.
#'
#' @noRd
#' @param fry.dyr the output directory from `alevin-fry quant ...`
#' @param ng number of genes that were quantitated
#' @return a list of alevin-fry data primitives:
#'   - `$counts`: a sparse matrix of counts
#'   - `$genes`: character vector of gene names (rows)
#'   - `$barcodes`: character vector of barcode names (columns)
#'   - `$usa`: a named list of three integer vectors that are column indices into
#'      `$counts` that hold the values for the `S`pliced, `U`unspliced, and
#'      `A`mbiguous count estimates
.loadFry <- function(fry.dir, ng) {
