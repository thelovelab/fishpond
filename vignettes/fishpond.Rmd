...
the secondary covariate labels are permuted while keeping the main
condition labels fixed. For unmatched samples, the difference in the
condition log2 fold change across the secondary covariate is used as
the test statistic, and the permutation scheme just described is
used. The motivation for the above permutation schemes is to ensure
the following condition, that "under the null hypothesis, the
likelihood of the data is invariant under these permutations"
...
We begin the interaction analysis by re-reading in the data, now
including all of the samples, and not just the control and IFN gamma
stimulated samples as before:
...
We create two new factors indicating IFNg status and *Salmonella* status:

```{r}
coldata$ifng <- factor(ifelse(
  grepl("IFNg",coldata$condition),
  "treated","control"))
coldata$salmonella <- factor(ifelse(
  grepl("SL1344",coldata$condition),
  "infected","control"))
with(coldata,
...
Now we read in the quantification data using `tximeta`:
...
set.seed(1)
for (i in 1:3) {
  id.perms <- c(sample(id[condition==1],n),
                sample(id[condition==2],n))[c(1:4,9:12,5:8,13:16)]
