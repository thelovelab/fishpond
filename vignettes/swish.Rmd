...
variance, and optionally the inferential replicate counts. `tximeta`
...
importing the inferential replicates is desirable.
...
look across real scRNA-seq data, we load the neurons (900) dataset and
...
We can easily make a single cell experiment, and plot counts across
cluster (here a randomly assigned cluster label):
...
`plotInfReps` has a number of convenience arguments, including the
ability to add a `legend`, to `reorder` the cells by expression value
within cluster, to apply size factors to the counts (`applySF`), and
whether to use a column of `mcols(sce)` to set the main title
(`mainCol`). See `?plotInfReps` for more description of arguments.
Size factor scaling will use the values in `sizeFactors(sce)` or
equivalently `mcols(sce)$sizeFactor`.

Note that the figures scale to some degree by the number of cells; for
example with $n \in [1,150)$ or $[150,400)$, varying visual elements
...
calculation function. We have found that `computeSumFactors`
...
`scaleInfReps` from the code chunk above.
