...
# Running swish with output from *alevin*

The *alevin* [@alevin] and *tximport* / *tximeta* maintainers have
created an efficient format for storing and importing the sparse 
scRNA-seq estimated gene counts, and optionally inferential mean and
variance, and the inferential replicate counts. `tximeta` will
automatically  import these matrices if *alevin* was run using
...
replicates themselves, see below on thoughts on optionally avoiding
this step though).
...
replicates, which are generated using the inferential mean and
variance matrices. Storing and importing only the inferential mean and
variance is much more efficient (dramatically faster load time and
less space on disk and in memory). The faster workflow would then skip
`--dumpFeatures` above, or use `dropInfReps=TRUE` to not load the
inferential replicates into R. We describe both workflows below.

`swish` can be run on *alevin* counts imported with `tximeta`, but
there are a few extra steps suggested:
...
**Filter cells:** One should also subset to the groups of cells of
...
calculation function. One could use `computeSumFactors` in the *scran*
package for example. The code chunk below provides the `"poscounts"`
size factor estimation function that was used in the *Swish* paper.
...
**(Faster) Without inferential replicates:** If one has avoided
storing the inferential replicates, or avoided loading them into R,
then we first need to generate pseudo-inferential replicates from
inferential mean and variance matrices before running `scaleInfReps`
...
jobs, and running `swish` across batches of genes at a time. This
helps to reduce the total memory used, in the case that the counts,
mean, and variance matrices are too large to be made dense
altogether. This scheme involves 1) splitting the object into smaller
pieces, written out to `.rds` files, 2) running `swish` as a
...
```{r, eval=FALSE}
# after filtering genes and cells...

# compute and store size factors calculated over all genes
cts <- as.matrix(assays(y)[["counts"]])
sf <- sfFun(cts)
