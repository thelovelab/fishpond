...
However, additional designs within the nonparametric testing framework
of *Swish* (and originally *SAMseq*) can be explored. This testing
framework is to compute a test statistic, average that over
inferential replicates, and then permute the samples in order to
generate a null distribution of the averaged test statistic. The
*qvalue* method and package is then used to return the final
inference. Here we explore testing outside of `swish` function, by
following these steps. We will test for differential correlation of
...
of log2 fold changes. It is useful to define a function to repeat this
operation (over inferential replicates and permutations).

```{r}
computeStat <- function(mat, design) {
  lfc <- log2(mat[,idx1] + pc) - log2(mat[,idx2] + pc)
  fit <- limma::lmFit(lfc, design)
  fit$coefficients[,"groupB:cov"]
}
```
...
stat <- rowMeans(sapply(reps, function(k) computeStat(assay(y, k), design)))
```
...
for (p in seq_len(nperms)) {
  permuted_design <- design[sample(n),]
  nulls[,p] <- rowMeans(sapply(reps, function(k) {
    computeStat(assay(y, k), permuted_design)
  }))
}
```
